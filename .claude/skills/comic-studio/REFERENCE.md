...
1. **创建**：创建弹窗「📖 漫画」tab——正文 .txt（UTF-8）+ 页数（0=按剧情密度自动）/**百万像素档**（0.3~1.6，旧 WxH 自动换算；0.4≈30s/页·Krea2 耗时闸）/质量档/对白呈现（bubble/footer/none+气泡透明度字色字号）；画风可选 Krea2 风格库（中文名主显）
2. **🚀 一键出片**：分析→✓确认资产→自动角色主图→✓主图满意→拆解（规则1b 剧情基准/6b 姿势衔接/6 禁复读）→逐页生成→「漫画就绪」自动停（最后一页落盘自推进，不依赖 autopilot）
3. **页面生成道**（参考页三优先级）：**Krea2 快道**（comic_page_krea2 主力：图1场景/图2人物槽序固定、LoraStack Detail+Realism、百万像素直传、步数固定10；**双角色双方案** comic_dual_mode——总数>2 才处理：stitch 拼接默认/chain 链式可切，≤2 直种）→ zimage_page_ref（Edit-2511 多图）→ 纯 t2i 文字锚。页间连续性：延续页注入上页姿势摘要+seed 库值
4. **浏览/微调**：详情「漫画页」页签（点击放大←→翻页、多选批量、单卡🔄重出/💬单页重排）；改对白呈现/气泡样式 →「💬 重排对白」秒级本地（干净副本 page_NNN_clean.png，不重出图）；**气泡定位**：肤色+边缘能量六分区自动避让，不满意「✥ 拖气泡」手动拖位（存 ledger.bubble_pos 即时生效，↺回落自动）
5. **导出**：📄 导出 PDF（Pillow，需 `pip install -e ".[pdf]"`）/ 📜 导出长图 → `output/comic.pdf|comic_strip.png`；无效镜不进导出
6. **🎬 转视频**：漫画就绪后变身视频项目（设置面板：**三类型** standard 重渲染/motion 动态漫[活动页换干净副本去气泡]/film 漫改；画风预填可改+视频参数全可调）——分镜/对白/资产复用，分镜描述自动生成视频提示词（不读图，不行🔄强制重读）→autopilot 渲染→合成；漫画页保留可导出。**单行道**：转完漫画页 UI 消失，先满意再转
...
**注意**：资产更新（外貌编辑/主图重生）不标 stale（页面是成品）——日志提示「绑定 N 页建议重出」；comic_output 项目详情只显示漫画链按钮（视频链按钮全隐藏——批量渲染/快车道/配音/读图/时长重估/角色配音等；拆分分镜/门1门2/参考图/补绑角色/批量无效保留）；导引页按「视频项目/漫画项目」两大区展示全部操作链，每卡「➕ 去创建」直跳预选。角色参考图分析/生成照跑但页面生成不消费（角色一致性=二期参考图注入）；gen_comic_page 不在重启重排白名单——重启丢在跑页任务用手动补页；subtitles 恒 0（对白页面自呈）。
...
**模型切换**：manifest `models:` 槽位 → settings model_overrides（键=模板 id）→ filler 注入；choices 从 /object_info 枚举。**LoRA 开关槽**（2026-09-14）：槽位加 `switch:` 字段（如 `switch: 启用_1`）——设置页选「（关闭）」存空串 → filler 只关开关不换文件名；未覆盖=模板内置（krea_t2i 与 comic_page_krea2 的 8 LoRA 栈均适用）。**krea_t2i**（Lazy_Krea2_文生图）：纯文生图道（width/height 注入、百万像素不注入）；unet/vae/clip/8 LoRA 全可切。**模板级步数**（`settings.template_params`，按模板各自设）：作用于主图/参考图（genref）与漫画页 Krea2 快道；漫画页纯 t2i/参考注入走项目质量档。设置页模板映射按产线分组（图像生成/视频渲染/修整），漫画页三槽（快道/参考注入/纯 t2i）在 UI 可换。
