...
- **Krea2 快道 `comic_page_krea2`（2026-09-13 用户实测定稿·主力道）**：用户手工验证 Lazy_Krea2 全模式工作台——**百万像素 1.0→0.4 后双参考 90s→30s、单参考 50s→20s 且质量满意风格贴合**（Krea2=风格库原生底模）。模板复制 krea_page_redraw 结构（2000 Workbench→2001 Generate），slots：图1=场景（scene，无则灰）/ 图2=人物（char）；**双角色左右拼接合成参考**（`_stitch_mains` 等高拼接+mtime 哈希缓存——identity_edit LoRA 双图训练上限的社区合法手法，prompt 说明「图2 左侧 X 右侧 Y」）；`megapixels` 按项目尺寸档换算（512x768≈0.39 即 0.4 手感）、步数走质量档；Krea2 模板缺失回落 zimage_page_ref。**参考页路由优先级：krea2 → zimage_page_ref → 纯 t2i**。注意 2001 的百万像素字段是可选输入（api.json 显式写入才可注入）；prompt 长度影响 Edit-2511 编码耗时（~0.22s/字实测）
