...
- Phase 2 角色音色系统（2026-08-30 全链落地）— `voices.py`（15 预设英文映射+pace 语速+禁词+两级匹配：LLM suggested_voice 优先→性别×年龄 8 档基线兜底）；`voicelib.py`（generate_preset 走 **qwen_tts_design** 模板 / process_upload 走 **qwen_tts_clone**（上传+裁剪起止+默认句克隆）/ list_voices 双作用域 origin）；两张 TTS 模板=用户实测工作流落库；h3_ref2va 接 LoadAudio×2（manifest audio0/audio1 槽，filler 后缀分流→ComfyClient.upload_media /upload/audio）；渲染 `_voice_slots_for_shot`：dialogue 说话人→assets.voice→样本入槽+`<Audio N>` 声明前置+标 `ledger.h3_native_voice`→合成跳过 TTS（保 H3 原声口型）；迁移 29 assets.voice；API routes_voices（列表/上传/预设生成/删除/绑定）+ 资产 PATCH voice + 主题 PATCH 编辑；前端设置页「🎵 音色库」tab（生成/⟳重生成/内嵌试听）、资产弹窗配音音色下拉、项目级上传、「🎵 项目音色」入口；音色样本缓存 data/voices/{presets,custom}+projects/<slug>/voices；ComfyUI 侧 zzz-qwen3tts-shim（transformers 5.x rope 'default' 注入，抗换包）
