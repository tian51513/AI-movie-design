...
- **漫画页间连续性三件套（2026-09-13 用户需求：前后页姿势/构图突变）**：①COMIC_SPLIT_RULES 规则 6b——同场景延续页描述**必须写明姿势/位置衔接**（「仍坐在床沿，转头看向门口」），姿势突变仅限场景断点/时间跳跃页（治源头，新拆解生效）②`_prev_page_summary`（seq-1 生效镜 description 前 80 字）+ continuity∈{全程继承,微变延续} 门控——生成时注入「与上一页画面连续：上一格{摘要}——姿势位置衔接变化，不要突兀跳变」（治存量，重出即吃）③comicgen **seed 库值优先**（shots.seed 拆解时延续组 +3 已算好——同组页渲染风格/角色样貌稳定；无库值才随机）。不做上页成图参考/latent（v1.3 底图锁死判例）
