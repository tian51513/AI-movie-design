...
- **Krea2 快道 `comic_page_krea2`（2026-09-13 用户实测定稿·主力道）**：用户手工验证 Lazy_Krea2 全模式工作台——**百万像素 1.0→0.4 后双参考 90s→30s、单参考 50s→20s 且质量满意风格贴合**（Krea2=风格库原生底模）。模板复制 krea_page_redraw 结构（2000 Workbench→2001 Generate），slots：图1=场景（scene，无则灰）/ 图2=人物（char）——**槽序固定**（README：scene 恒 image1/person 恒 image2，交换即劣化）。**双角色链式两段**（官方 workaround：P1 场景+A → 中间产物作 P2 的 image1 再插 B——拼接合成参考有「两人脸趋同」官方确认缺陷，弃用）；`megapixels` 按项目尺寸档换算（512x768≈0.39 即 0.4 手感）、步数走质量档；Krea2 模板缺失回落 zimage_page_ref。**参考页路由优先级：krea2 → zimage_page_ref → 纯 t2i**。注意 2001 的百万像素字段是可选输入（api.json 显式写入才可注入）；prompt 长度影响 Edit-2511 编码耗时（~0.22s/字实测）。**同日用户栈同步**（_raw/Lazy_Krea2全模式工作台.json）：补 `LazyKreaLoraStack`（启用 PornMaster Detail+Realism 双 LoRA，模型/底模双输出接线；**turbo_4step 关**——用户实测 4 步=10 步=30s，瓶颈在 Qwen3-VL 接地/编码/双图 VAE 固定开销非采样步数，蒸馏 LoRA 零收益纯画质风险）+ 接地像素 512/参考强度 2/场景强度 1/CFG 1；**krea 道步数固定 10**（质量档映射无意义）。上传 500 判例：ComfyUI 编辑/重载窗口期后端短暂拒上传（ASCII/中文名与同名连传复现全通过），避免边改工作流边出页
