...
- **气泡智能定位 + 转化类型三选 + 干净副本/对白重排（2026-09-13 晚批量）**：①`_bubble_layout` 六分区（左上/右上/左中/右中/左下/右下）**边缘能量择空**（`_zone_energies`：Pillow FIND_EDGES 缩 64×96 分区均值升序——人物/细节=高频避让，用户观察「左上或中部常空看站位」）；最空分区先放、分区用尽按能量序叠、放不下回分区顶；无图回落左右交替顶部流（旧测试不破）。②转化 `video_mode` 三选：standard=重渲染（comic_mode=''）/ **motion=动态漫**（原画动起来——活动页**换干净副本去气泡**，首尾帧不带字对白走 TTS+字幕）/ film=漫改；前端 c2vMode 下拉。③干净副本 `page_NNN_clean.png`（后处理前落盘）+ `POST rebubble-comic`（💬 重排对白：改 dialogue_mode/气泡样式秒级本地重排不重出图；旧页无副本跳过计数）；副本=未来动态漫无气泡首尾帧源。**定位 v2（第4页真机判例：平滑肤色脸边缘能量低于细致背景被误判最空→气泡盖脸）**：能量=边缘均值+0.6×灰度std+**60×肤色占比**（r>g>b 启发式，251+ 排除气泡白自身）×分区位置权重（顶角 0.85/中带 1.35——构图主体常居中），128×192 采样。**手动拖位（用户需求：自动不满意时人工拖）**：ledger.bubble_pos=[[x,y]…]（页像素、按对白序号），`POST /api/shots/{id}/bubble-pos` 保存+立即从干净副本重排生效（409 无副本）；前端「✥ 拖气泡」编辑器（pointer 拖动、缩放比换算、↺ 回落自动=清空覆盖位）；rebubble-comic 扩 shot_ids 单页重排+reset_positions
- **漫画页间连续性三件套（2026-09-13 用户需求：前后页姿势/构图突变）**：①COMIC_SPLIT_RULES 规则 6b——同场景延续页描述**必须写明姿势/位置衔接**（「仍坐在床沿，转头看向门口」），姿势突变仅限场景断点/时间跳跃页（治源头，新拆解生效）②`_prev_page_summary`（seq-1 生效镜 description 前 80 字）+ continuity∈{全程继承,微变延续} 门控——生成时注入「与上一页画面连续：上一格{摘要}——姿势位置衔接变化，不要突兀跳变」（治存量，重出即吃）③comicgen **seed 库值优先**（shots.seed 拆解时延续组 +3 已算好——同组页渲染风格/角色样貌稳定；无库值才随机）；**显式重出例外**（`payload.reshuffle_seed`，真机「点重出没变化」判例：同 seed 同提示词输出雷同）——force/shot_ids 路由自动带，随机新 seed 并回写库值（update_shot 白名单补 seed），autopilot 补缺路径不带保连续。不做上页成图参考/latent（v1.3 底图锁死判例）
- **转化动态漫首尾帧=原漫画页接线（2026-09-14 真机：已渲两镜与漫画零关联——此前未接 kf，fl2v 发现缺失自动 t2i 生成了全新画面）**：convert 路由 motion 模式打完干净副本后接线——页 i=镜 i kf_start、页 i+1=镜 i kf_end（翻页链同导入约定，**末镜无尾帧→渲染降级 i2v 仅首帧**）；只接干净副本、只补缺失（重转化幂等、手工放的 kf 不覆盖）；ensure_keyframes 见文件在即跳过=永不自动生成。调整分镜数量后新增镜无对应页→回落自动生成（或手动放 kf）
- **转化项目跳 VLM 读图（迁移 41 `comic_converted`·2026-09-14 用户决策）**：漫画流转的视频项目（三类型通用）缺提示词 → **gen_prompts 按分镜描述直生成**（读图=拿图片倒推库里已有的一手文字，纯浪费；「🔄强制重读」留手动兜底）；正常漫画导入项目（comic_converted=0）仍走 describe_shots 原流程不受影响。`_comic_flow` ① 分支内转化判定在 describe_shots 之前，gen_prompt 在飞/失败守卫同款
- **前端两修（2026-09-14 真机）**：①`kreaNameZh` 误放 computed 段——模板按函数调用（`kreaNameZh().slice()`），Vue computed 解包成字符串 → `TypeError: not a function` 渲染崩溃整页黑屏（选 Krea2 风格确认即触发；挪 methods 修）。**判例：模板函数调用的助手必须进 methods，computed 只放属性语义**。②七个创建方法「`alert(); return` 早退跳过末尾 `creating=false`」——服务端 422（如正文非 UTF-8）后按钮永久「创建中」灰死；全部改 `finally { this.creating = false; }`（createProject 补 try/finally、createFromTheme 同款加固）。**判例：flag 复位必须 finally，错误分支 return 是隐形跳过点**
- **转化面板复刻漫画导入全参 + 模式联动（2026-09-14 用户需求）**：补段时长/总时长/是否重绘角色三参（复刻创建弹窗漫画导入参数集——画幅/兆像素/倍速/质量档/字幕/渲染模式/画风原有）；**模式联动**：c2vMode 切换自动定渲染默认（motion→fl2v 首尾帧翻页/film→ref2va 角色动画/standard→ref2va）+ 提示词模式 motion/film 下禁用（describe_shots 读图链生成 integrated/六段格式，A~E 不消费）+ 行内消费性提示；用户手改渲染模式后（_c2vRenderTouched）再切模式不覆盖。注意：update_video_params 的**总时长>0 会均摊并同步段时长**（2026-08-26 既有设计）——转化想保留段时长就别传总时长
- **stale 联动 comic_output 豁免 + 漫画终态自推进（2026-09-13 真机判例×2）**：①编辑角色外貌 → `mark_stale_for_asset` 把 27 绑定镜标 stale → 漫画页 UI 只剩 2 页可见（用户以为数据丢）——**页面是成品不因资产更新失效**：comic_output 项目豁免 stale 标记，改提示日志「绑定 N 个漫画页（页号…）建议重出，页面文件未受影响」；前端 stale 态也**显示图**+标签「资产已更新·建议重出」（不藏图占位）。②手动批量重出（autopilot 关）全页齐但 stage 停 storyboard_ready →「🎬 转视频」按钮按 comic_ready 门控不可见——gen_comic_page 最后一页落盘即查全生效镜缺页、零缺自动 `set_stage comic_ready`+收工日志（终态推进不依赖 autopilot）
...
- **导演台真机闭环（2026-09-19 凌晨）**：17 镜 ~2 分钟成片整段跑通（~1 小时，含每批 RTX 超分）。上游判例：核心 ComfyUI 更新把 `MiniMaxH3ReferenceToVideo.execute` 的 vae/audio_vae 挪可选尾参——导演台包 `conditioning.py` 旧位置传参错位（height 收到提示词字符串 → `height // 16` TypeError），本地补丁改关键字传参（包更新自带修复不冲突）；**ComfyUI Desktop 关窗口≠退进程**（"重启"后栈帧行号还是旧文件=模块没重载，要托盘 Quit 或杀 python.exe）
