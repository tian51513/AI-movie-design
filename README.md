...
- [x] Phase 6：配音字幕（Edge-TTS 配音 + SRT 字幕烧录，合成自动混入）
- [x] 分镜控制（2026-08-27）：拆分镜可指定分镜数（不填自动）；镜头可批量置
  无效/生效/删除——无效镜渲染与合成跳过；对白机械补录（模型不填 dialogue 时从原文引号恢复）
- [x] LLM 对接硬化（2026-08-27）：Ollama/LM Studio base_url 自动归一；思考模型响应处理；测试连接不再假通过
- [x] 提示词方言（2026-08-27）：模板级 prompt_style——SD 系 CLIP 模板走英文标签流，Z-Image qwen 编码器走中文；画风拆层（视觉子集进图像、叙事词只进视频提示词）；写实意图增强
- [x] 体验升级（2026-08-27）：主题生成两步流（预览确认+补充描述）；创建入口顶部弹窗；项目列表瀑布流/分页双视图+富信息
- [x] 🎧 有声书导入（P10A 2026-09-05）：上传音频自动转写（faster-whisper）回填正文，之后走小说链；**分镜时长=音频实测段长**（自动覆盖一切估时）；转写未完成点一键出片会被守卫拦住（wait/409）。需装 asr extra（`.venv/bin/pip install -e ".[asr]"`，首次下载 large-v3 ~3GB，可传本地模型路径省流量）——**转写期间全队列串行（单 worker），长书先切分或临时调 workers>1；running 转写不可中断**
- [x] 🖌 动态漫角色重绘（2026-09-09 建；2026-09-12 Krea2 快道+风格预设库）：漫画导入勾「是否重绘角色」——VLM 提取主要角色→按画风重绘角色主图（Krea2 风格库 73 库预览选择）→停等检查满意后「🖌 批量重绘分镜」整页重绘（Krea2 快道 10-15s/页；清气泡文字/尾帧联动/首尾帧版本可切换缩略图秒级）→自动续跑渲染合成
- [x] 📖 小说转漫画（2026-09-12 建；09-13 大修）：上传正文（.txt/有声书转写/主题生成三入口）→资产停等确认→角色主图停等检查→拆解（剧情基准/姿势衔接/防复读）→**Krea2 快道逐页生成 20-30s/页**（双参考图+画风LoRA栈+智能双角色方案）+干净副本→「漫画就绪」；**气泡系统**：肤色+边缘能量自动避让人物、✥手动拖位、💬重排秒级换样式；百万像素档尺寸；页间连续性（上页姿势摘要+seed继承）；📄PDF/📜长图导出；**🎬转视频**（重渲染/动态漫/漫改三选，全数据复用）

### 有声书流程（P10A）

上传音频（≤200MB）→ 等转写 job 完成（日志「转写完成：N 段 / M 字」、正文不再是占位文本）→ 检查转写人名（错名会导致角色对不上）→ 🚀 一键出片（与小说同链）。当前配音走 TTS；**原声切片直用属 P10B 未做**。模型缓存位置由 `HF_HOME`/`HF_HUB_CACHE` 决定，勿随意清除该变量否则重新下载。
...
- 「⏹ 停止自动」= 联动全停（2026-09-05）：关开关同时取消排队任务、打断运行中任务——随时回手动改内容，再点一键出片从当前状态续跑
- 失败守卫：拆分镜/读图/合成失败不自动重烧（日志报一次「autopilot 卡住」等手动处理）；单镜提示词/渲染失败跳过该镜推进其余
- 在飞感知：重渲染未收尾不门3、不合成（旧视频不会抢拼进成片）
- 详情页 ⚡ 角标实时显示当前动作；分镜区「🎬 合成成片」也可手动触发
- 断点对账：渲染中途重启时，已提交 ComfyUI 且已完成的镜头直接落盘，不重渲
- 停止语义（2026-09-05 温和化）：⏹ 停止任务/🗑 清空队列/删除项目均**不打断 ComfyUI 在跑任务**（部分版本 interrupt 会崩实例）——取消待跑、在跑的自然跑完落盘
- 对白时长保障（2026-09-05）：时长按字数/4 秒估；合成时配音若仍长于视频，末帧定格自动补长——对白永远说得完，字幕随音频轴对齐
...
## 关键帧链路（fl2v，2026-08-25 方案A）

分镜 workflow 选「首尾帧生视频 fl2v」后，渲染时自动：

1. 缺关键帧则用文生图模板生成 `kf_start.png` / `kf_end.png`——两帧**同一 seed**（构图一致），提示词带成对约束（同机位/景别/光线，仅动作表情不同）+ 画风/时代 + ZImage 尾缀
2. 视频提示词自动追加「镜内禁切、首帧平滑过渡至尾帧」约束（和解 D 模式的镜内多镜递进）
3. 走 `h3_fl2v` 首尾帧插值生成；关键帧生成失败自动降级为仅首帧模式（h3_i2v）

已有 `kf_*.png` 的分镜不会重生成（可手工替换文件换帧）。角色一致性另配「多参考生视频 ref2va」。
