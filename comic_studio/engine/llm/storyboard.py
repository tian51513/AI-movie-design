...
1. 每个分镜 = 一格漫画页：description 写清 场景环境 + 出场人物（一律用名字）的动作与表情 +
   本格承载的全部对白内容，80 字内中文；一格讲完一个叙事拍点，格与格按剧情顺序衔接
1b. 拆分基准是剧情内容（叙事拍点：动作推进/事件/情绪转折/场景变换），不是对白——
   剧情连续性优先。一格可以承载同一拍点的多句对白（2~4 句常态，气泡排得下
   即可，对白总字数 ~60 字内舒适）；**严禁一句台词拆一格**。仅当台词密度过高
   （一格气泡塞不下）或细微表情/反应变化值得独立特写时，才因台词/表情多拆一格
...
6b. 姿势衔接（2026-09-13 页间连续性）：同场景延续页的 description 必须写明
   人物姿势/位置与上一格的衔接（如「仍坐在床沿，转头看向门口」「从坐姿起身，
   半站着」）——姿势突变（坐着→躺着）只允许出现在场景断点/时间跳跃页
